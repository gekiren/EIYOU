...
## 6. AIモデル指定規約
- **Gemini APIモデルの指定**: アプリケーション（Cloudflare Workers プロキシ経由、直接API呼出等）から呼び出す栄養解析およびAIチャット用のGeminiモデルには、必ず **`gemini-3.6-flash`** を優先・固定して使用すること。過去のモデルや他モデルへの不用意な書き換えは禁止とする。
